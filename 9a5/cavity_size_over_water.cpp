...
int main() {
    ...
    for (int nc = start_nc; nc != end_nc + 1; ++nc) {
        ...
        std::vector<index> O_WAT_IN_C_id_mid_lowerlayer;
        ...
        for (int frame = 0; frame < total_frame; frame+=relaxation_time){
            ...
            if(frame!=total_frame) {
                O_WAT_IN_C_id_mid_lowerlayer.clear();
                ...
                for (index i = 0; i != O_WAT_id.size(); ++i) {
                    ...
                    if (O_coor[2] <  (Z_UP + Z_DOWN) /2 +2 && O_coor[2] > Z_DOWN && O_coor[0] < X_UP &&
                        O_coor[0] > X_DOWN && O_coor[1] < Y_UP && O_coor[1] > Y_DOWN)//做修改，将水分为两层
                    {
                        O_WAT_IN_C_id_mid_lowerlayer.push_back(O_WAT_id[i]);
                ...
                for (int i=0; i<O_WAT_IN_C_id_mid_lowerlayer.size();i++) {
                    Target_ID=O_WAT_IN_C_id_mid_lowerlayer[i];
                    //std::cout<<Target_ID<<std::setw(10)<<nc<<std::setw(10)<<frame<<std::endl;
                        for (int i = 0; i < x_points; i++) {
                            for (int j = 0; j < y_points; j++) {
                                calc_matrix[i][j] = 0;
                            }
        ...
        outfile.open("cavity_jump_correlation/cutoff_1a3_average_cavitysize_by_z_dum");
        outfile2.open("cavity_jump_correlation/cutoff_1a3_largecavity_prob_by_z_dum");
