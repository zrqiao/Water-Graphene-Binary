...
int main() {
    ...
    static double Nor_distribution[z_points][90]={0};
    static double H2_averz_t [t_com*2]={0};
    static double H1_averz_t [t_com*2]={0};
    static double O_averz_t [t_com*2]={0};
    static double O1_averz_t [t_com*2]={0};
    static double O2_averz_t [t_com*2]={0};
    static double H2_averangle_t [t_com*2]={0};
    static double H1_averangle_t [t_com*2]={0};
    static double O2H2_averangle_t [t_com*2]={0};
    static double O1H1_averangle_t [t_com*2]={0};
    ...
    infile0.open("H-Bond/H_Bond_O_index_transition_donor_up_zeropoint_1a2_TST");
    ...
    int dtp;
    while (!infile0.fail()) {
        infile0 >> zero_num[0] >> zero_num[1] >> zero_num[2] >> zero_num[3] >> zero_num[4] >> zero_num[5]>>zero_num[6]>>zero_num[7];
        jump_count=zero_num[0];
        int nc0=zero_num[2],frame0=zero_num[3];
        int O1_ID=zero_num[6],O2_ID=zero_num[7];
        ...
        while (!infile.fail()) {
            ...
            HB_ID2 = num[5];
                ...
                sprintf(name_nc, "nc/density_dis9a5_%d.nc", nc);
                nctraj nc_data(name_nc);
                O_coor = nc_data.atom_coordinate(frame, Target_ID);
                O1_coor=nc_data.atom_coordinate(frame,O1_ID);
                O2_coor=nc_data.atom_coordinate(frame,O2_ID);
                ...
                O1H1_vec = vector_calc::vector_minus(O1_coor,H1_coor);
                O2H2_vec = vector_calc::vector_minus(O2_coor,H2_coor);
                Dip_vec = vector_calc::vector_minus(vector_calc::vector_divide(vector_calc::vector_add(H1_coor,H2_coor),2),O_coor);
                Nor_vec = vector_calc::cross_product(OH2_vec,OH1_vec);
                angleH1=calc_angle_z_axis(OH1_vec);
                angleH2=calc_angle_z_axis(OH2_vec);
                angleO1H1=calc_angle_z_axis(O1H1_vec);
                angleO2H2=calc_angle_z_axis(O2H2_vec);
                ...
                total_wat[int(round((O_coor[2] - Z_DOWN) / dz))]++;
                total_wat_t[dtp]++;
                H1_averz_t[dtp]+=H1_coor[2];
                H2_averz_t[dtp]+=H2_coor[2];
                O_averz_t[dtp]+=O_coor[2];
                O1_averz_t[dtp]+=O1_coor[2];
                O2_averz_t[dtp]+=O2_coor[2];
                H1_averangle_t[dtp]+=angleH1;
                H2_averangle_t[dtp]+=angleH2;
                O1H1_averangle_t[dtp]+=angleO1H1;
                O2H2_averangle_t[dtp]+=angleO2H2;
                Dipole_averangle_t[dtp]+=angleDip;
                Normal_averangle_t[dtp]+=angleNor;
                Nor_distribution[int(round((O_coor[2] - Z_DOWN) / dz))][int(floor(angleNor))]++;
                if (num[temp_H1+3] != -1) {
                    H1O_coor = nc_data.atom_coordinate(frame, num[temp_H1+3]);
                    if (judge_layer(H1O_coor, Z_DOWN, Z_UP) == 1) {
                        H1_avernum_upperlayer[int(round((O_coor[2] - Z_DOWN) / dz))]++;
                        H1_avernum_upperlayer_t[dtp]++;
                    } else if (judge_layer(H1O_coor, Z_DOWN, Z_UP) == -1) {
                        H1_avernum_lowerlayer[int(round((O_coor[2] - Z_DOWN) / dz))]++;
                ...
                if (num[temp_H2+3] != -1) {
                    ...
                    if (judge_layer(H2O_coor, Z_DOWN, Z_UP) == 1) {
                        H2_avernum_upperlayer[int(round((O_coor[2] - Z_DOWN) / dz))]++;
                        H2_avernum_upperlayer_t[dtp]++;
                        H2_correlationfunction_t[dtp]+=1;
                    } else if (judge_layer(H2O_coor, Z_DOWN, Z_UP) == -1) {
                        H2_avernum_lowerlayer[int(round((O_coor[2] - Z_DOWN) / dz))]++;
    ...
    outfile.open("jump/cutoff_1a2/H1H2_z_averHbondnum_cutoff1a2_up_TST");//0 z 1 H1u 2 H1l 3 H2u 4 H2l
    std::ofstream outfile1;
    outfile1.open("jump/cutoff_1a2/H1H2_t_averHbondnum_cutoff1a2_up_TST");//0 t 1 H1u 2 H1l 3 H2u 4 H2l
    std::ofstream outfile2;
    outfile2.open("jump/cutoff_1a2/H1H2_t_averz_cutoff1a2_up_TST");//0 z 1 H1 2 H2 3 O
    std::ofstream outfile3;
    outfile3.open("jump/cutoff_1a2/H1H2_t_averangle_zaxis_cutoff1a2_up_TST");//0 t 1 H1 2 H2 3 D 4 N
    std::ofstream outfile4;
    outfile4.open("jump/cutoff_1a2/H1H2_t_Hbond_layerexchange_correlationfunction_TST");
    ...
    for(int i =1; i !=t_com*2; ++i)
    {
        ...
        outfile2<<4*(i*dt-t_com*dt)<<std::setw(12)<<H1_averz_t[i]/total_wat_t[i]<<std::setw(12)<<H2_averz_t[i]/total_wat_t[i]
                <<std::setw(12)<<O_averz_t[i]/total_wat_t[i]<<std::setw(12)<<O1_averz_t[i]/total_wat_t[i]<<std::setw(12)<<O2_averz_t[i]/total_wat_t[i]<<std::endl;
        outfile3<<4*(i*dt-t_com*dt)<<std::setw(12)<<H1_averangle_t[i]/total_wat_t[i]<<std::setw(12)<<H2_averangle_t[i]/total_wat_t[i]
                <<std::setw(12)<<Dipole_averangle_t[i]/total_wat_t[i]<<std::setw(12)<<Normal_averangle_t[i]/total_wat_t[i]<<std::setw(12)
                <<O1H1_averangle_t[i]/total_wat_t[i]<<std::setw(12)<<O2H2_averangle_t[i]/total_wat_t[i]<<std::endl;
    ...
    outfile5.open("jump/cutoff_1a2/H1H2_z_Nor_angledistribution_TST");
    for(int i =1; i !=z_points; ++i){
        for (int j=0;j<90;j++){
            outfile5<<Nor_distribution[i][j]/total_wat[i]<<std::setw(12);
        }
        outfile5<<std::endl;
    }
    outfile.close();
