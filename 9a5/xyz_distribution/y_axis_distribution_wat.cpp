...
int main()
{
	...
	std::cout << "program to calculate the y_axis distribution"<< "\n"<< std::endl;
	std::cout << "calculate XYZ_limit" << std::endl;
	
	std::vector<int> y_axis_dis(y_axis_num,0);
	...
	nctraj data_nc("density_dis9a5_12.nc");
	...
	for(int nc = start_nc; nc  != end_nc+1; ++nc)
	{
       ...
       while(frame 	<     total_frame)
       {
		    for(index i = 0; i != O_WAT_id.size(); ++i)
		    {
				...
				{
					step = (Y_UP-Y_DOWN)/y_axis_num;
				    y_axis_dis[floor((o_coor[1] - Y_DOWN)/step)] += 1;
				}
			}
		   frame += 3;
	   }
...
//~ 
	//~ 
	//~ outfile.open("y_axis_distribution");
//~ 
	for(index i = 0; i != y_axis_dis.size(); ++i)
	{
		//~ outfile <<std::setw(15) <<i<<std::setw(15)<<z_axis_dis[i] << std::endl;
		std::cout <<std::setw(15) <<i<<std::setw(15)<<y_axis_dis[i] << std::endl;
     }
     //~ outfile.close();      
	return 0;
}
