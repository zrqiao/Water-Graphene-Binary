...
int main()
{
	...
	double total_wat =0;
	int bond_distribution[bond_length_number][bond_angle_number]={0};
				  ...
				  for(index j =0; j != select_wat_id.size(); ++j)
				  {
					  if( O_WAT_IN_C_id[i] != select_wat_id[j] )
					  {
						  ...
						  if(hbond_cutoff_down<distance && distance < hbond_cutoff_up)
						  {
							  ...
							  {
								  ++bond_distribution[int(floor((distance-hbond_cutoff_down)/((hbond_cutoff_up-hbond_cutoff_down)/bond_length_number)))][int(floor((180.0*acos(angle)/vector_calc::PI -  hbond_cutoff_angle_down)/((hbond_cutoff_angle_up-hbond_cutoff_angle_down)/bond_angle_number)))];	         
							  }
						  }
					  }
				  }
    ...
    for(int i =0; i !=bond_angle_number; ++i)
    {
		for(int j=0; j != bond_length_number; ++j)
		{
			outfile<<std::setw(9)<<bond_distribution[j][i];
		}
		outfile<<std::endl;
	} 
