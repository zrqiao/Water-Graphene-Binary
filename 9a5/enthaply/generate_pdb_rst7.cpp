    ...
    for(int frame = 0; frame < 10000; frame += delta_frame)
    {
	    ...
	    for( index i = 0 ; i != O_WAT_id.size() ; ++i)                                                             
    	{
	     ...
	     if(frame == 0)
	     {
			 ...
			 outfile3.open("wat_c_pdb");
			 for(index i = 0; i != 999; ++i)
             {
		         outfile3 <<"ATOM"<<std::setw(7) <<i+1<<std::setw(5)<<"A"<<i+1<<std::setw(7)<<"gra"<<std::setw(7)<<1<<std::setw(15)
             ...
             {
		          outfile3 <<"ATOM"<<std::setw(7) <<i+1<<std::setw(5)<<"B"<<i-999<<std::setw(7)<<"gra"<<std::setw(7)<<1<<std::setw(15)
		                          <<nc_data.atom_coordinate(frame, graphene_id[i])[0] << std::setw(15)<<nc_data.atom_coordinate(frame, graphene_id[i])[1]<<
		                         std::setw(15)<<nc_data.atom_coordinate(frame, graphene_id[i])[2]<<std::endl;
	         }
            ...
            {
		            outfile3 <<"ATOM"<<std::setw(7) <<i+1<<std::setw(5)<<"A"<<i -1399<<std::setw(7)<<"gra"<<std::setw(7)<<2<<std::setw(15)
		             <<nc_data.atom_coordinate(frame, graphene_id[i])[0] << std::setw(15)<<nc_data.atom_coordinate(frame, graphene_id[i])[1]<<
		             std::setw(15)<<nc_data.atom_coordinate(frame, graphene_id[i])[2]<<std::endl;
	        }
	        for(index i = 2399; i != 2800; ++i)
            {
		           outfile3 <<"ATOM"<<std::setw(7) <<i+1<<std::setw(5)<<"B"<<i -2399<<std::setw(7)<<"gra"<<std::setw(7)<<2<<std::setw(15)
		                          <<nc_data.atom_coordinate(frame, graphene_id[i])[0] << std::setw(15)<<nc_data.atom_coordinate(frame, graphene_id[i])[1]<<
		                           std::setw(15)<<nc_data.atom_coordinate(frame, graphene_id[i])[2]<<std::endl;
	        }
	       outfile3<<"TER"<<std::endl;	 
	       ...
	       {
		        int judge = 0;
		        for(int j =0; j!= min_count_wat_in_c; ++j)
		        {
		        ...
		        {  
			         ++sum;
			         outfile3 <<"ATOM"<<std::setw(7) <<3*(sum-1)+2801<<std::setw(5)<<"O"<<std::setw(7)<<"WAT"<<std::setw(7)<<(sum-1)+3<<std::setw(15)
		             ...
		             outfile3 <<"ATOM"<<std::setw(7) <<3*(sum-1)+2802<<std::setw(5)<<"H1"<<std::setw(7)<<"WAT"<<std::setw(7)<<(sum-1)+3<<std::setw(15)
		             ...
		             outfile3 <<"ATOM"<<std::setw(7) <<3*(sum-1)+2803<<std::setw(5)<<"H2"<<std::setw(7)<<"WAT"<<std::setw(7)<<(sum-1)+3<<std::setw(15)
	    ...
	    sprintf(name_1,"/media/cxx/disk3/data_water_in_graphene/280k/9a5/enthaply/222/wat_graphene_9a5_%d.rst7",frame);
	    outfile1.open(name_1);
	    outfile1.setf(ios::fixed, ios::floatfield);
	    outfile1.precision(7);
	    outfile1<<"default_name"<<std::endl;
	    outfile1<<nc_data.atoms_number()<<std::endl;
	    //~ std::cout << "atoms_number:  "<<nc_data.atoms_number()<<std::endl;
	    
	    for(index i  = 0; i != nc_data.atoms_number(); ++i)
	    {
			if((i%2) == 0)
			{
					outfile1<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[0]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[1]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[2];
		    }
			if((i%2) != 0)
			{
					outfile1 <<std::setw(12)<<nc_data.atom_coordinate(frame,i)[0]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[1]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[2]<<std::endl;
			}
		}
		if( (nc_data.atoms_number()%2) != 0)outfile1<<std::endl;
        ...
        sprintf(name_2, "/media/cxx/disk3/data_water_in_graphene/280k/9a5/enthaply/222/wat_c_9a5_%d.rst7",frame);
         outfile2.open(name_2);
         outfile2.setf(ios::fixed, ios::floatfield);
         outfile2.precision(7);
         outfile2<<"default_name"<<std::endl;
         outfile2<<nc_data.atoms_number()-3*min_count_wat_in_c<<std::endl;
         //~ std::cout<<nc_data.atoms_number()<<"  "<<min_cout_wat_in_c<<" "<< nc_data.atoms_number()-3*min_cout_wat_in_c<<std::endl;
	     ...
	     {     
		     ...
		     for(int j =0; j!= min_count_wat_in_c; ++j)
		     {
			     if((i == O_WAT_IN_C_id[j]) || (i == (O_WAT_IN_C_id[j]+1)) || (i == (O_WAT_IN_C_id[j]+2)) )
			     {
				     judge  = 1;   		     
			     }
		     }
		     if(judge == 0)
		     {
			    if((sum%2) == 0)
		    	{
					outfile2<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[0]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[1]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[2];
		        }
			    if((sum%2) != 0)
		    	{
					outfile2<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[0]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[1]<<std::setw(12)<<nc_data.atom_coordinate(frame,i)[2]<<std::endl;
		        }		          
	    ...
	    if((sum%2) !=  0) outfile2<<std::endl;
		outfile2 <<std::setw(12)<<nc_data.cell_lengths(frame_cell_lengths)[0]<<std::setw(12)<<nc_data.cell_lengths(frame_cell_lengths)[1]<<std::setw(12)<<nc_data.cell_lengths(frame_cell_lengths)[2]
		                <<std::setw(12)<<90.0000000<<std::setw(12)<<90.0000000<<std::setw(12)<<90.0000000<<std::endl;
        outfile2.close();     
