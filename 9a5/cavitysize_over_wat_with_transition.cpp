...
int calc_cavity_size(int &circumference, int x, int y, int &last_step_size, int **judge_matrix, int **calc_matrix,
    ...
    if (calc_matrix[x][y]==0){
        ...
        if (x-1>=0){
            if (judge_matrix[x-1][y]==1 && calc_matrix[x-1][y]==0){
                ...
                calc_cavity_size(circumference, x - 1, y, last_step_size, judge_matrix, calc_matrix, x_points, y_points);
            }
        }
        if (y-1>=0){
            if (judge_matrix[x][y-1]==1 && calc_matrix[x][y-1]==0){
                coordinate_vector[0]=x;
                coordinate_vector[1]=y;
                calc_cavity_size(circumference, x, y - 1, last_step_size, judge_matrix, calc_matrix, x_points, y_points);
            }
        }
        if (x+1<x_points){
            if (judge_matrix[x+1][y]==1 && calc_matrix[x+1][y]==0){
                ...
                for (int i=0; i<x_points;i++){//深搜统计新的空穴大小和数目
                    for(int j=0;j<y_points;j++){
                        if (judge_matrix[i][j]==1&& calc_matrix[i][j]==0){
                            ...
                            calc_cavity_size(cavity_circ, i, j, cavity_size, judge_matrix, calc_matrix, x_points,
                                             y_points);//
