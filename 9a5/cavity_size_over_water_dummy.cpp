...
int main() {
    ...
    for (int nc = start_nc; nc != end_nc + 1; ++nc) {
        ...
        for (int frame = 0; frame < total_frame; frame+=relaxation_time){
            ...
            if(frame!=total_frame) {
                ...
                for (int j=0; j<y_points;j++){
                    ...
                    int x=int(x_points/2),y=int(y_points/2);
                    if (0<=x&&x<x_points && 0<=y&&y<y_points) {
                        ...
                        for (int i=0;i<z_points;i++) {
                            total_wat_z[i]++;
                            cavity_size_sum[i] += size;
                            //std::cout<<size<<std::endl;
                            if (size >= large_cutoff)
                                large_cavity_probability[i]++;
                        }
                    }
        ...
        outfile.open("cavity_jump_correlation/cutoff_1a3_average_cavitysize_by_z_fixed");
        outfile2.open("cavity_jump_correlation/cutoff_1a3_largecavity_prob_by_z_fixed");
